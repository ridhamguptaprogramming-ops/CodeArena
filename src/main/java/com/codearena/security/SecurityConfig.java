...
public class SecurityConfig { 
    ...
    var rateLimitFilter=new RateLimitFilter(redis); var jwtFilter=new JwtFilter(jwt);
    h.csrf(c->c.disable()).cors(c->c.configurationSource
            (req->{var x=new CorsConfiguration();x.setAllowedOriginPatterns(Arrays.stream(origins.split(",")).map(value -> value == null ? "" : value.trim()).toList());x.setAllowedMethods(List.of("GET","POST","PUT","PATCH","DELETE","OPTIONS"));x.setAllowedHeaders(List.of("Authorization","Content-Type"));x.setAllowCredentials(true);return x;})).sessionManagement(s->s.sessionCreationPolicy(SessionCreationPolicy.STATELESS)).authorizeHttpRequests(a->a.requestMatchers("/api/v1/auth/register","/api/v1/auth/login","/api/v1/auth/refresh","/api/v1/auth/logout","/actuator/health").permitAll().requestMatchers(HttpMethod.GET,"/api/v1/problems/**").permitAll().requestMatchers("/ws/**").permitAll().anyRequest().authenticated()).addFilterBefore(rateLimitFilter,UsernamePasswordAuthenticationFilter.class).addFilterAfter(jwtFilter,RateLimitFilter.class); return h.build(); }
static class RateLimitFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(@NonNull HttpServletRequest req,@NonNull HttpServletResponse res,@NonNull FilterChain chain)throws ServletException,IOException {
        if(req.getRequestURI().startsWith("/api/v1/auth/")&&req.getMethod().equals("POST")) {
            ...
            try { Long n=redis.opsForValue().increment(key); if(n!=null&&n==1)redis.expire(key,java.util.Objects.requireNonNull(Duration.ofMinutes(1))); if(n!=null&&n>20){res.setStatus(HttpStatus.TOO_MANY_REQUESTS.value());return;} }
